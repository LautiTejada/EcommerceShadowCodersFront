...
export const ModalAgregarTalleProduct = ({ producto, onClose }: ModalAgregarTalleProductProps) => {
  ...
  const tallesDisponibles = talles.filter(
    (talle: Talle) =>
      talle.activo &&
      !producto.talles?.some((pt) => pt.talle.id === talle.id)
  );
  ...
  return (
    <div className={styles.modalOverlay}>
      <div className={styles.modalContent}>
        <h2 className={styles.title}>Agregar Talle</h2>
        <form onSubmit={handleSubmit} className={styles.form}>
          ...
          <select
            ...
            onChange={(e) => {
              ...
              const talle = tallesDisponibles.find((t) => t.id === selectedId) || null;
